...
func _spawn_generators() -> void:
    var specs := [
        ...
        ["C", "Nest Override", Vector2(350, 780), Color(0.95, 0.55, 0.22, 1.0)]
    ]
    for spec in specs:
...
func _spawn_mimics() -> void:
    # Mimics removed from project; function kept as no-op for compatibility
    return
...
func _play_intro_cutscene() -> void:
    ...
    GameManager.run_state = "cutscene"
    
    var tw = create_tween()
    tw.tween_interval(2.0)
    tw.tween_property(bg, "modulate:a", 0.0, 1.5)
    tw.parallel().tween_property(label, "modulate:a", 0.0, 1.5)
    tw.tween_callback(func():
        GameManager.run_state = "playing"
        canvas.queue_free()
    )
